...
评分解释以 preset 为准：
- `short_term_momentum`：使用基座+亮点动作规则；`Base>=68 && HL>=2` 为 Strong Watch，`Base>=58 && HL>=1` 为 Setup Watch，`Base>=45 && HL>=1` 为 Alert，`Base<45` 为 Avoid。
- `long_term_compounder`：使用基座+亮点动作规则；`Base>=65 && HL>=2` 为 Thesis Candidate，`Base>=55 && HL>=1` 为 DCF Candidate，`Base>=40` 进入 Watchlist，`Base<40` Reject。

这些分层用于候选优先级，不是买卖信号。小样本 thesis 池需要同时看相对排名。
...
| Preset | 用途 | 核心权重 |
|--------|------|----------|
| `short_term_momentum` | 短线 1-4 周观察 / 试仓候选 | 趋势、市场状态、低风险买点、相对强度、成交确认、陷阱过滤 |
