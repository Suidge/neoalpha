...
│   ├── screen_momentum.py
│   ├── screen_stocks.py
│   ├── backtest_screener.py            # 历史滚动窗口滑动仿真回测验证脚本
│   ├── build_dcf_model.py
│   ├── validate_dcf.py
...
每个 monitor 至少包含 `symbol`, `name`, `focus`, `triggers[]`。所有个股相关可读汇报必须同时显示代码 and 股票名称。HK 策略允许 `.HK/.SZ/.SH`，用于覆盖同段交易的 A 股。
...
## Preset 选股器 v3.3.4 双层架构

为了解决传统多因子加权平均评分导致个股亮点信号被稀释、机会捕捉差和追高陷阱的问题，选股器系统在 v3.3.0 进行了重构，并在 **v3.3.4** 加入低风险买点、市场状态和假突破/出货过滤，全面采用**基座+亮点双层架构**：

1. **基座层 (Foundation Layer — 风控底线)**
   - 提取趋势、动量、相对强度、流动性、筹码/量价质量、基本面等核心因子计算加权基座分（0-100）。
   - **过度延伸风控惩罚 (Overextension Penalty)**：在 v3.3.3 中引入了价格偏离均线（MA20>8% 或 MA50>15%）、连续上涨（5-7天）或高位震荡的扣分，设定严格的基座合格线（短线从 50 分调至 45 分，长线 40 分）。未达合格线标的直接判定为 `Avoid` (短线) 或 `Reject` (中长线)，实行硬性防御。
   - **市场状态与假突破过滤 (v3.3.4)**：新增 `market_regime` 和 `false_breakout_distribution_risk`，将指数趋势、冲高回落、放量滞涨、突破失败和派发日纳入基座风控，防止单一技术亮点在弱市场或出货结构里误升级。

2. **亮点层 (Highlight Layer — 机会与前瞻触发)**
   - 包含单针洗盘、布林收缩、K线反转、简化缠论背驰、VCP 波动收缩等 10+ 个具有高胜率的择时亮点。
   - **低风险买点 (Low-Risk Entry, v3.3.4)**：识别上升趋势中回踩 MA20/MA50/白线/BBI 支撑、缩量吸筹、收盘重新站回支撑且风险未扩散的可买位置，减少被月度动量滞后低估的有效机会。
   - **前瞻性蓄势信号 (Pre-Breakout Setup)**：在 v3.3.3 中新增了 **窄幅收敛蓄势 (Tight Base Setup)**、**缩量机构吸筹 (Volume Dry Pocket)** 以及 **突破前高张力 (Pre-Breakout Tension)** 三种捕捉临界供需平衡的前瞻亮点，让买点预测提前 1-3 天。
   - **双阶段突破计算**：将 `breakout_ignition` 分拆为 **Pre-Ignition (蓄势期)** 与 **Ignition (确认期)**，支持在阻力位 2-5% 以内提前埋伏。

3. **大批量性能优化与防限流**
   - **冗余合并**：月线历史数据拉取合并为单次 CLI 接口请求。
   - **本地缓存**：在 `~/.cache/neoalpha/` 针对日线及月线数据分别引入了 12 小时的文件缓存机制，暖缓存重复拉取零接口调用，耗时从 3 分钟降至 2 秒左右。
   - **安全限速**：严格单线程串行执行，冷缓存时自动插入 0.2s 延时，彻底杜绝 Longbridge 行情 429 限流风险。
   - **Smart Cache Bypass (v3.3.1)**：盘中若扫描标的数量少于 10 支，会自动强制 cache 存活时间为 0.0，实现 100% 实时穿透分析。
...
1. **基座层 (Foundation Layer — 风控底线)**
   - 提取趋势、动量、相对强度、流动性、筹码/量价质量、基本面等核心因子计算加权基座分（0-100）。
   - 设定严格的基座合格线（短线 45 分，长线 40 分）。未达合格线标的直接判定为 `Avoid` (短线) 或 `Reject` (中长线)，从风控上进行硬性防御，过滤空头或高风险个股。
