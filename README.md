...
### 3. SMAM 动量扫描与 Preset 选股器 (v3.3.4 低风险买点+陷阱过滤架构)

NeoAlpha 内置全新升级的 **双层选股器评分引擎**，可从 thesis 标的、持仓池或自定义标的分组中进行大批量扫描：

- **基座风控与市场状态过滤**：对趋势、市场状态、动量、相对强度、流动性、筹码质量、低风险买点、题材及催化计算加权基座分，并引入 **过度延伸惩罚 (Overextension Penalty)** 与 **假突破/出货风险 (False Breakout / Distribution Risk)**。当价格偏离 MA20 (>8%)、MA50 (>15%)、连续上涨、放量滞涨、冲高回落或突破失败时扣分，未达合格线（短线 45 分，长线 40 分）直接风控拦截（Avoid / Reject）。
- **低风险买点与前瞻蓄势触发**：不仅保留单针洗盘、布林收缩 (TTM Squeeze)、K线反转、简化缠论背驰、VCP 波动收缩等择时亮点，还加入 **低风险买点 (Low-Risk Entry)**、**窄幅收敛蓄势 (Tight Base)**、**缩量机构吸筹 (Volume Dry Pocket)** 与 **突破前高张力 (Pre-Breakout Tension)**，重点寻找上升趋势中回踩支撑、缩量吸筹和突破前的可控风险位置。
- **实盘历史回测校准**：`scripts/backtest_screener.py` 支持市场过滤、正确基准映射和同标的 cooldown 去重，可用真实 Longbridge 日线回测短线信号的 5/10/20 日胜率、平均收益、利润因子与持有期最大回撤。
...
python3 skills/neoalpha/scripts/screen_momentum.py --from-thesis
python3 skills/neoalpha/scripts/screen_stocks.py --from-thesis --preset short_term_momentum
python3 skills/neoalpha/scripts/backtest_screener.py --from-thesis --market CN --signal all --lookback-days 230 --cooldown-days 5
python3 skills/neoalpha/scripts/portfolio_ledger.py record "<trade description>"
...
- [references/technical-screener.md](references/technical-screener.md)：v3.3.4 选股器技术指标定义
- [references/screener-usage.md](references/screener-usage.md)：选股器及回测框架运行指南
