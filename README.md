...
### 3. SMAM 动量扫描与 Preset 选股器

NeoAlpha 内置 SMAM 动量框架和 preset 选股器，可以从 thesis 标的、持仓池或手工 symbols 中批量扫描：

- 短线动量强弱
- 趋势延续与回撤状态
- 价格、成交量和均线结构
- `Strong Watch` / `Setup Watch` / `Thesis Candidate` / `DCF Candidate` 等后续动作标签
...
将技能目录复制到 OpenClaw skills 目录。目录名与技能 slug 统一为 `neoalpha`。
...
更多 cron 配置与验证命令见 [INSTALLATION.md](INSTALLATION.md)。
