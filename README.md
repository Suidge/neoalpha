...
### 3. SMAM 动量扫描与 Preset 选股器 (v3.3.3 基座风控+前瞻亮点架构)

NeoAlpha 内置全新升级的 **双层选股器评分引擎**，可从 thesis 标的、持仓池或自定义标的分组中进行大批量扫描：

- **基座风控与价格乖离过滤**：对趋势、动量、相对强度、流动性、题材及基本面核心因子计算加权基座分，并引入了 **过度延伸惩罚 (Overextension Penalty)**。当价格偏离 MA20 (>8%)、MA50 (>15%) 或连续上涨时进行强力扣分，未达合格线（短线 45 分，长线 40 分）直接风控拦截（Avoid / Reject），杜绝高位追高。
- **前瞻蓄势与独立亮点触发**：不仅保留了单针洗盘、布林收缩 (TTM Squeeze)、K线反转、简化缠论背驰、VCP 波动收缩等高胜率择时亮点，更追加了 **窄幅收敛蓄势 (Tight Base)**、**缩量机构吸筹 (Volume Dry Pocket)** 以及 **突破前高张力 (Pre-Breakout Tension)** 等前瞻性亮点，一有触发即刻标记，提前 1-3 天锁定爆发买点。
...
python3 skills/neoalpha/scripts/screen_momentum.py --from-thesis
python3 skills/neoalpha/scripts/screen_stocks.py --from-thesis --preset short_term_momentum
python3 skills/neoalpha/scripts/backtest_screener.py --from-thesis --signal all --lookback-days 120
python3 skills/neoalpha/scripts/portfolio_ledger.py record "<trade description>"
...
- [references/technical-screener.md](references/technical-screener.md)：v3.3.3 选股器技术指标定义
