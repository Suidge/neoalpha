...
class DCFModelValidator:
    ...
    def _check_terminal_growth_vs_wacc(self):
        ...
        try:
            ...
            dcf_formulas = self.workbook_formulas['DCF']
            ...
            for row in dcf_sheet.iter_rows(max_row=100, max_col=20):
                for cell in row:
                    label_value = cell.value if cell.value is not None else dcf_formulas[cell.coordinate].value
                    if label_value and isinstance(label_value, str):
                        cell_str = label_value.lower()
                        if 'terminal' in cell_str and 'growth' in cell_str:
                            ...
                            for offset in range(1, 5):
                                ...
                                if adjacent is None:
                                    adjacent = dcf_formulas.cell(cell.row, cell.column + offset).value
                                if ((isinstance(adjacent, (int, float)) and 0 < adjacent < 1) or (isinstance(adjacent, str) and adjacent.startswith('='))):
            ...
            if terminal_growth is not None and wacc is not None:
                if isinstance(terminal_growth, (int, float)) and isinstance(wacc, (int, float)):
                    ...
                    self.info.append("Found terminal growth and WACC formulas; open workbook in Excel/LibreOffice to recalculate values")
            else:
    ...
    def _check_wacc_range(self):
        ...
        try:
            ...
            wacc_formulas = self.workbook_formulas['WACC'] if 'WACC' in self.workbook_formulas.sheetnames else self.workbook_formulas['DCF']
            wacc = None

            for row in wacc_sheet.iter_rows(max_row=100, max_col=20):
                for cell in row:
                    label_value = cell.value if cell.value is not None else wacc_formulas[cell.coordinate].value
                    if label_value and isinstance(label_value, str):
                        label = label_value.lower()
                        if 'wacc' in label and 'weight' not in label:
                            for offset in range(1, 5):
                                adjacent = wacc_sheet.cell(cell.row, cell.column + offset).value
                                if adjacent is None:
                                    adjacent = wacc_formulas.cell(cell.row, cell.column + offset).value
                                if ((isinstance(adjacent, (int, float)) and 0 < adjacent < 1) or (isinstance(adjacent, str) and adjacent.startswith('='))):
                                    wacc = adjacent
                                    break
            ...
            if wacc is not None:
                if isinstance(wacc, (int, float)):
                    ...
                    self.info.append("Found WACC formula; open workbook in Excel/LibreOffice to recalculate value")
            else:
    ...
    def _check_terminal_value_proportion(self):
        ...
        try:
                    ...
                    if label_value and isinstance(label_value, str):
                        ...
                        if 'terminal' in cell_str and 'value' in cell_str and 'pv' in cell_str:
                            for offset in range(1, 5):
                                ...
                                if adjacent is None:
                                    adjacent = dcf_formulas.cell(cell.row, cell.column + offset).value
                                if ((isinstance(adjacent, (int, float)) and adjacent > 0) or (isinstance(adjacent, str) and adjacent.startswith('='))):
            ...
            if terminal_value is not None and enterprise_value is not None:
                if isinstance(terminal_value, (int, float)) and isinstance(enterprise_value, (int, float)) and enterprise_value > 0:
                    ...
                    self.info.append("Found terminal value and enterprise value formulas; open workbook in Excel/LibreOffice to recalculate values")
            else:
